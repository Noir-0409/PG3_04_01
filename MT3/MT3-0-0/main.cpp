...
void DrawBezier(const Vector3& controlPoint0, const Vector3& controlPoint1, const Vector3& controlPoint2,
	...
	for (int i = 1; i <= numSegments; ++i) {
		...
		Vector3 transformedPrevPoint = Transform(previousPoint, viewProjectionMatrix);
		transformedPrevPoint = Transform(transformedPrevPoint, viewPortMatrix);

		Vector3 transformedPoint = Transform(point, viewProjectionMatrix);
		transformedPoint = Transform(transformedPoint, viewPortMatrix);

		Novice::DrawLine(static_cast<int>(transformedPrevPoint.x), static_cast<int>(transformedPrevPoint.y),
			static_cast<int>(transformedPoint.x), static_cast<int>(transformedPoint.y), color);
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	Vector3 cameraRotate{ 0.14f, 0.0f, 0.0f };
	...
	while (Novice::ProcessMessage() == 0) {
		...
		ImGui::DragFloat3("Points0", &controlPoints[0].x, 0.01f);
		ImGui::DragFloat3("Points1", &controlPoints[1].x, 0.01f);
		ImGui::DragFloat3("Points2", &controlPoints[2].x, 0.01f);
		ImGui::End();
