...
const char kWindowTitle[] = "MT3-2-6";
...
//
//	Vector3 clossestPoint{ std::clamp(sphere.center.x,aabb.min.x,aabb.max.x),
//		(sphere.center.y,aabb.min.y,aabb.max.y),
//		(sphere.center.z,aabb.min.z,aabb.max.z)
//	};
//
//	float distance = Length(clossestPoint - sphere.center);
//
...
bool IsCollision(const AABB& aabb, const Sphere& sphere) {
	...
	// 最近接点とスフィアの中心との距離を計算
	Vector3 distanceVector = closestPoint - sphere.center;
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	Sphere sphere{ {0.0f, 0.5f, 0.0f}, 0.3f };

	AABB aabb{ .min{-0.3f,-0.3f,-0.3f},.max{0.0f,0.0f,0.0f} };
	...
	///	Matrix4x4 viewProjectionMatrix = Multiply(viewMatrix, projectionMatrix);
		...
		Novice::ScreenPrintf(10, 10, "IsCollision=%d", IsCollision(aabb, sphere));
