...
const char kWindowTitle[] = "MT3-2-4";
...
struct Triangle {
	...
	Vector3 vertices1; //頂点
	Vector3 vertices2;
	Vector3 vertices3;

};

bool IsPointInTriangle(const Vector3& p, const Triangle& triangle) {
	Vector3 v0 = Subtract(triangle.vertices3, triangle.vertices1);
	Vector3 v1 = Subtract(triangle.vertices2, triangle.vertices1);
	Vector3 v2 = Subtract(p, triangle.vertices1);
...
bool IsCollision(const Triangle& triangle, const Segment& segment) {
	...
	Vector3 p1 = { segment.origin.x + segment.diff.x, segment.origin.y + segment.diff.y, segment.origin.z + segment.diff.z };

	// Check if either end of the segment is inside the triangle
	if (IsPointInTriangle(p0, triangle) || IsPointInTriangle(p1, triangle)) {
		return true;
	}

	// Check for intersections between the segment and the triangle's edges
	Vector3 edges[3][2] = {
		{triangle.vertices1, triangle.vertices2},
		{triangle.vertices2, triangle.vertices3},
		{triangle.vertices3, triangle.vertices1}
	};

	for (int i = 0; i < 3; ++i) {
		Vector3 edgeStart = edges[i][0];
		Vector3 edgeEnd = edges[i][1];
		Vector3 edgeDiff = Subtract(edgeEnd, edgeStart);

		Vector3 s1 = Subtract(edgeStart, p0);
		Vector3 s2 = Subtract(edgeEnd, p0);

		Vector3 d1 = Cross(segment.diff, edgeDiff);
		Vector3 d2 = Cross(s1, edgeDiff);
		Vector3 d3 = Cross(s2, edgeDiff);

		if (Dot(d1, d2) * Dot(d1, d3) < 0) {
			return true;
		}
	}
...
Vector3 Closestpoint(const Vector3& point, const Segment& segment) {
	...
	Vector3 segmentEnd = Vector3{ segment.origin.x + segment.diff.x,
	...
	// 線分の始点から点へのベクトルを計算
	Vector3 segmentToPoint = Vector3{ point.x - segmentStart.x,
									  point.y - segmentStart.y,
									  point.z - segmentStart.z };

	// 線分の長さを計算
	float segmentLengthSquared = segmentDirection.x * segmentDirection.x +
		segmentDirection.y * segmentDirection.y +
		segmentDirection.z * segmentDirection.z;

	// 線分の長さが 0 に近い場合 (線分が点の場合)
	if (segmentLengthSquared < 1e-6) {

		return segmentStart; // 線分の始点を返す

	}

	// 線分上での最も近い点までのスカラー値を計算
	float t = (segmentToPoint.x * segmentDirection.x +
		segmentToPoint.y * segmentDirection.y +
		segmentToPoint.z * segmentDirection.z) / segmentLengthSquared;

	// t が 0 より小さい場合、点が線分の始点よりも前にある
	if (t < 0.0f) {

		return segmentStart; // 線分の始点を返す

	}
	// t が 1 より大きい場合、点が線分の終点よりも後ろにある
	else if (t > 1.0f) {
		return segmentEnd; // 線分の終点を返す
	}
	// 線分上に点がある場合、線分上の最も近い点を計算して返す
	else {

		Vector3 closestPoint;
		closestPoint.x = segmentStart.x + t * segmentDirection.x;
		closestPoint.y = segmentStart.y + t * segmentDirection.y;
		closestPoint.z = segmentStart.z + t * segmentDirection.z;
...
void DrawTriangle(const Triangle& triangle, const Matrix4x4& viewProjectionMatrix, const Matrix4x4& viewPortMatrix, uint32_t color) {
	...
	// 各頂点を変換
	Vector3 screenA = Transform(Transform(triangle.vertices1, viewProjectionMatrix), viewPortMatrix);
	Vector3 screenB = Transform(Transform(triangle.vertices2, viewProjectionMatrix), viewPortMatrix);
	Vector3 screenC = Transform(Transform(triangle.vertices3, viewProjectionMatrix), viewPortMatrix);
