...
void DrawTriangle(const Triangle& triangle, const Matrix4x4& viewProjectionMatrix, const Matrix4x4& viewPortMatrix, uint32_t color) {
	...
	for (int i = 0; i < 3; ++i) {
		...
		Vector3 clipSpaceVertex = Transform(triangle.vertices[i], viewProjectionMatrix);

		Vector3 ndcSpaceVertex = Transform(clipSpaceVertex, viewPortMatrix);

		transformedVertices[i] = ndcSpaceVertex;
	}

	Novice::DrawTriangle(
		(int)transformedVertices[0].x, (int)transformedVertices[0].y,
		(int)transformedVertices[1].x, (int)transformedVertices[1].y,
		(int)transformedVertices[2].x, (int)transformedVertices[2].y,
...
bool IsCollision(const Triangle& triangle, const Segment& segment) {
	...
	// 線分の端点から三角形の各頂点へのベクトルを計算
	Vector3 v0p = Subtract(segment.origin, triangle.vertices[0]);
	Vector3 v1p = Subtract(segment.origin, triangle.vertices[1]);
	Vector3 v2p = Subtract(segment.origin, triangle.vertices[2]);
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	while (Novice::ProcessMessage() == 0) {
		...
		/*for (int i = 0; i < 3; i++) {
			...
			triangle.vertices[i] = Normalize(triangle.vertices[i]);
		...
		ImGui::DragFloat3("v0", &triangle.vertices[0].x, 0.01f);
		ImGui::DragFloat3("v1", &triangle.vertices[1].x, 0.01f);
		ImGui::DragFloat3("v2", &triangle.vertices[2].x, 0.01f);
		//ImGui::InputFloat3("Project", &project.x, "%.3f", ImGuiInputTextFlags_ReadOnly);
		ImGui::End();

		Novice::ScreenPrintf(10, 10, "IsCollision=%d", IsCollision(triangle, segment));
